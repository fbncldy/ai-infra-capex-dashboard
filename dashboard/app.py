...
with tab_overview:
    ...
    st.markdown(
        ...
        f"services-heavy.\n"
        f"- **Underneath it all, scarcity is compressing at every layer.** The "
        f"three things AI made scarce, compute, model capability and the "
        f"ability to build software, are all getting cheaper at once: "
        f"inference prices fall about 10x a year, benchmark scores have "
        f"converged in the 90s, and code generation has collapsed build "
        f"costs. The rents attached to each compress with them. What stays "
        f"valuable is what was scarce before AI: distribution, proprietary "
        f"data, regulatory position and switching costs, which is why almost "
        f"every player is integrating vertically (matrix below) to hold ground "
        f"the technology no longer protects. The telecom precedent rhymes but "
        f"is not identical, because here the infrastructure produces the "
        f"intelligence itself and the incumbents already span the stack, so "
        f"disruption from outside is harder than the wave that commoditized "
        f"carriers."
    )
